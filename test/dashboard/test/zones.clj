...
(defn set-zone-id!
  "Set the id of zone with name"
  [db-conn name id]
  (raw-sql-update
   db-conn
   (str "UPDATE  `ebdb_test`.`zones` SET  `id` =  '" id "' WHERE  "
        "`zones`.`name` = '" name "';")))
...
;;         ;; set san-diego id to 3
;;         _ (set-zone-id! db-conn "San Diego" 3)
;;         ;; create La Jolla
...
;;         ;; set La Jolla id to 324
;;         _ (set-zone-id! db-conn "La Jolla" 324)
;;         ;; create El Cajon
...
;;         ;; set El Cajon id to 326
;;         _ (set-zone-id! db-conn "El Cajon" 326)]
;;     ;; (is (not= (:zips (zones/get-zone-by-id db-conn 337))
;;     ;;           "Zip"
;;     ;;           ))
;;     ))
